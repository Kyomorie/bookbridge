...
MODEL = r'''class PendingRewind(Base):
...
replace_once('src/db/models.py', 'class PendingSuggestion(Base):\n', MODEL + 'class PendingSuggestion(Base):\n')
replace_once(
...
METHODS = r'''    # Pending rewind approval operations (KoSync -> ABS only)
    @staticmethod
...
replace_once(
    ...
    METHODS + '    # PendingSuggestion operations\n',
)
replace_once(
...
migration = Path('alembic/versions/b4d7e9f2c1a6_add_pending_rewinds.py')
if migration.exists():
    raise SystemExit('migration path already exists')
migration.write_text(r'''"""add durable pending KoSync -> ABS rewind approvals
...
tests = Path('tests/test_pending_rewinds.py')
if tests.exists():
    raise SystemExit('test path already exists')
tests.write_text(r'''from datetime import datetime, timedelta
