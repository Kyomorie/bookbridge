...
class TestStoryGraphCooldown(unittest.TestCase):
    def setUp(self):
        ...
        self.mgr.sync_clients = {'StoryGraph': self.sg}
    ...
    def tearDown(self):
        os.environ.pop('STORYGRAPH_UPDATE_COOLDOWN_MINS', None)
    ...
    def test_posts_once_after_idle_then_no_duplicate(self):
        ...
        self.mgr._handle_storygraph_cooldown(self.book, self._config(0.5), now=3600.0)
        self.assertEqual(self.sg.calls, [0.5])
        # Saved StoryGraph state reflects the post.
        saved = self.db.get_state('book1', 'storygraph')
    ...
    def test_resume_within_cooldown_resets_timer(self):
        ...
        self.mgr._handle_storygraph_cooldown(self.book, self._config(0.55), now=1800.0)
        # t=2000: only 200s since the reset -> not settled, no post.
        self.mgr._handle_storygraph_cooldown(self.book, self._config(0.55), now=2000.0)
        self.assertEqual(self.sg.calls, [])

    def test_completion_bypasses_cooldown(self):
        # Completion on the very first observation, well inside the cooldown window.
        self.mgr._handle_storygraph_cooldown(self.book, self._config(0.995), now=0.0)
        self.assertEqual(len(self.sg.calls), 1)
        self.assertAlmostEqual(self.sg.calls[0], 0.995)

    def test_zero_cooldown_posts_immediately(self):
        os.environ['STORYGRAPH_UPDATE_COOLDOWN_MINS'] = '0'
        self.mgr._handle_storygraph_cooldown(self.book, self._config(0.5), now=0.0)
        self.assertEqual(self.sg.calls, [0.5])

    def test_unconfigured_client_is_skipped(self):
        self.sg._configured = False
        self.mgr._handle_storygraph_cooldown(self.book, self._config(0.995), now=0.0)
        self.assertEqual(self.sg.calls, [])
