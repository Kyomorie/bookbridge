...
class ABSSyncClient(SyncClient):
        ...
        if ts_for_text is not None:
            ...
            if abs_ts is not None and ts_for_text < abs_ts:
                ...
                updated_state = {
                    ...
                    # Private proposal metadata is intentionally not persisted as
                    # locator state; it exists only so SyncManager can offer the
                    # policy-skipped KoSync -> ABS rewind for explicit approval.
                    '_proposed_ts': float(ts_for_text),
                }
                service_updated_at = parse_service_timestamp(
                    response.get('lastUpdate') if response else None
                )
                if service_updated_at is not None:
                    updated_state['service_updated_at'] = service_updated_at
                return SyncResult(abs_ts, True, updated_state, skipped=True)
    ...
    def apply_approved_rewind(
        ...
        *,
        ...
        """Apply an approved rewind only if ABS still matches its snapshot."""
        response = self.abs_client.get_progress(book.abs_id)
        if response is None or response.get('currentTime') is None:
            return SyncResult(None, False)

        current_ts = float(response['currentTime'])
        service_updated_at = parse_service_timestamp(response.get('lastUpdate'))
        observed_state = {
            'ts': current_ts,
            'pct': self._abs_to_percentage(current_ts, book) or 0,
        }
        if service_updated_at is not None:
            observed_state['service_updated_at'] = service_updated_at

        target_matches = abs(current_ts - float(expected_current_ts)) <= 0.05
        service_matches = (
            expected_service_updated_at is None
            or (
                service_updated_at is not None
                and abs(
                    float(service_updated_at)
                    - float(expected_service_updated_at)
                ) <= 0.001
            )
        )

        if not target_matches or not service_matches:
            logger.info(
                "⏪ Approved ABS rewind became stale before write for '%s'; "
                "keeping live position %.2fs",
                book.abs_id,
                current_ts,
            )
            return SyncResult(
