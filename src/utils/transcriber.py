...
class AudioTranscriber:
    ...
    def split_duration(self) -> int:
        ...
        Read per access rather than cached in ``__init__``: the transcriber is a
        DI singleton, so a cached value would ignore settings changes until the
        next restart.
        """
        raw = os.environ.get("AUDIO_SPLIT_DURATION_MINUTES", "45")
        try:
            minutes = int(raw)
    ...
    def process_audio(
        ...
        MAX_DURATION_SECONDS = self.split_duration
